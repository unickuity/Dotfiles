_update_prompt () {
    ...
    e_title="\[\e]2;\u@\H \w\a\e[32;1m\]\[\e[0m\]"
	export PS1="$e_title$ex$_prompt$branch$e_white${BASH_PROMPT_PERLBREW}$e_normal $score$p ";
}
...
perlbrew_prompt() {
    if ! perlbrew >/dev/null 2>&1; then
        # no perlbrew here
        echo -n ""
    else
        perlbrew="$(perlbrew list | grep '^\*' | grep -v '(.*)' | sed -e 's/\* perl-//')"
        if [ -n "$perlbrew" ]; then
            export BASH_PROMPT_PERLBREW=" (perl $perlbrew)"
        else
            export BASH_PROMPT_PERLBREW=""
        fi
    fi
}
